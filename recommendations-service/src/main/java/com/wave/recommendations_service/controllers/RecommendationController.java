...
import com.wave.recommendations_service.components.RecruitmentClient;
...
public class RecommendationController {
    ...
    @PostMapping("/{vacancyId}") //TODO:: обрабатывать попытку получить рекомендации для вакансии, которая не принадлежит пользователю
    public Mono<List<RecommendationResult>> rank(
        @PathVariable UUID vacancyId,
        @RequestHeader("X-User-Id") UUID userId
    ) {
        return recruitmentClient.getApplicationsByVacancyId(vacancyId, userId).flatMap(vacancyApplications ->
            service.rank(vacancyApplications.vacancy(),
